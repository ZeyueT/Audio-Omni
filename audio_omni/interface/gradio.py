...
# Set temp directory
os.environ.setdefault('TMPDIR', '/tmp/audio_omni')
...
def _get_omni_conditioner(model):
    """
    Extract the OmniConditioner (and its .model / .processor) from a loaded
...
def _offload_non_omni_to_cpu(model):
    ...
    OmniConditioner (which owns the thinker we need for understanding).
    ...
    if pretransform is not None:
        try:
            ...
            pass
    # Move every conditioner except the OmniConditioner
    conditioner_wrapper = getattr(model, "conditioner", None)
    if conditioner_wrapper is not None:
        for _key, cond in getattr(conditioner_wrapper, "conditioners", {}).items():
            cls_name = type(cond).__name__
            if "OmniConditioner" not in cls_name and "QwenOmni" not in cls_name:
                try:
                    cond.cpu()
                except Exception:
                    pass
...
def generate_understanding(
    ...
    if omni_cond is None or omni_model is None or processor is None:
        err = "Error: the loaded model does not contain an OmniConditioner."
        chat_history.append({"role": "user", "content": text_input or "(empty)"})
        chat_history.append({"role": "assistant", "content": err})
        return chat_history, chat_history, ""
...
from ..prompts import build_prompt as _build_prompt

def build_qwen_prompt_t2a(prompt: str) -> str:
    return _build_prompt("T2A", prompt)

def build_qwen_prompt_t2m(prompt: str) -> str:
    return _build_prompt("T2M", prompt)

def build_qwen_prompt_tts(prompt: str) -> str:
    return _build_prompt("TTS", prompt)

def build_qwen_prompt_v2a(prompt: str) -> str:
    return _build_prompt("V2A", prompt)

def build_qwen_prompt_v2m(prompt: str) -> str:
    return _build_prompt("V2M", prompt)

def build_qwen_prompt_editing(prompt: str) -> str:
    from ..prompts import _wrap, SYSTEM_EDITING
    return _wrap(SYSTEM_EDITING, prompt)
...
def _auto_asr_voice_prompt(audio_path: str, model, device) -> str:
    """
    Use the OmniConditioner thinker to transcribe voice prompt audio via ASR.
    Returns the transcribed text, or empty string on failure.
    """
    try:
        omni_cond, processor, omni_model = _get_omni_conditioner(model)
        if omni_cond is None or omni_model is None or processor is None:
            print("[ASR] No OmniConditioner found, skipping auto ASR")
            return ""
...
def generate_cond(
    ...
    output_dir = f"output/{model_name}_prompt_{prompt[:10].replace(' ', '_')}"
    ...
    if video_path:
        merge_video_audio(video_path, output_audio_path, output_video_path, seconds_start, seconds_total)
    ...
    return (output_video_path, output_audio_path, audio_spectrogram)
...
def create_txt2audio_ui(model_config_map, default_model_name):
    with gr.Blocks() as ui:
        gr.HTML(
            ...
            '<h1>🎧 Audio-Omni</h1>'
            '<p style="font-size:1.1em; color:#555;">Audio-Omni: Extending Multi-modal Understanding to Versatile Audio Generation and Editing</p>'
            '<div style="display:flex; justify-content:center; gap:16px; flex-wrap:wrap; margin-top:8px;">'
            '<a href="https://zeyuet.github.io/Audio-Omni/" target="_blank" style="text-decoration:none;">'
            '<img src="https://img.shields.io/badge/Project-Page-blue?logo=Github&style=flat-square" alt="Project Page"></a>'
            '<a href="https://github.com/ZeyueT/Audio-Omni" target="_blank" style="text-decoration:none;">'
            '<img src="https://img.shields.io/badge/Code-GitHub-black?logo=github&style=flat-square" alt="Code"></a>'
            '<a href="https://arxiv.org/abs/XXXX.XXXXX" target="_blank" style="text-decoration:none;">'
            '<img src="https://img.shields.io/badge/arXiv-XXXX.XXXXX-brightgreen.svg?style=flat-square" alt="arXiv"></a>'
            '<a href="https://huggingface.co/HKUSTAudio/Audio-Omni" target="_blank" style="text-decoration:none;">'
            '<img src="https://img.shields.io/badge/%F0%9F%A4%97%20HuggingFace-Model-blue" alt="Model"></a>'
            '</div></div>'
        )
...
def create_ui(model_config_path=None, ckpt_path=None, pretrained_name=None, pretransform_ckpt_path=None, model_half=False):
    ...
    # Determine whether model_configs is a model-registry (mapping of name -> entry)
    # or a direct model config (contains "model_type", "model", etc.)
    is_direct_config = (
        isinstance(model_configs, dict) and "model_type" in model_configs
    )
    ...
    if is_direct_config:
        model_options = {
            "Audio-Omni": {
                ...
                "pretrained_name": pretrained_name,
                "pretransform_ckpt_path": pretransform_ckpt_path,
                "model_type": model_configs.get("model_type", "diffusion_cond"),
            }
        }
