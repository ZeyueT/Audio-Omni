...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def __init__(
            ...
            pretransform: tp.Optional[Pretransform] = None,
            high_level_cond_ids: tp.List[str] = [],
            low_level_cond_ids: tp.List[str] = [],
        ...
        self.pretransform = pretransform
        self.high_level_cond_ids = high_level_cond_ids
        self.low_level_cond_ids = low_level_cond_ids
    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[torch.Tensor, tp.Any], negative=False):
        ...
        if len(self.high_level_cond_ids) > 0:
            # Concatenate all cross-attention inputs over the sequence dimension
            # Assumes that the cross-attention inputs are of shape (batch, seq, channels)
            cross_attention_input = []
            cross_attention_masks = []

            for key in self.high_level_cond_ids:
        ...
        if len(self.low_level_cond_ids) > 0:
            # Concatenate all global conditioning inputs over the channel dimension
            global_conds = []
            for key in self.low_level_cond_ids:
...
def create_diffusion_cond_from_config(config: tp.Dict[str, tp.Any]):
    ...
    cross_attention_ids = diffusion_config.get('high_level_cond_ids', [])
    global_cond_ids = diffusion_config.get('low_level_cond_ids', [])
    ...
    return wrapper_fn(
        ...
        min_input_length=min_input_length,
        sample_rate=sample_rate,
        high_level_cond_ids=cross_attention_ids,
        low_level_cond_ids=global_cond_ids,
