...
torch.backends.cudnn.benchmark = False # fix algorithm selection
torch.backends.cudnn.deterministic = True # use deterministic implementation
torch.backends.cuda.matmul.allow_tf32 = False # disable TF32
torch.backends.cudnn.allow_tf32 = False 
torch.set_float32_matmul_precision("highest") # FP32 uses true FP32
...
def generate_diffusion_cond(
    ...
    # Handle different types of conditioning values: tensor, list, None
    def _convert_dtype(v):
        if v is None:
            return v
        elif isinstance(v, torch.Tensor):
            return v.type(model_dtype)
        elif isinstance(v, list):
            return [_convert_dtype(item) for item in v]
        else:
            return v
    conditioning_inputs = {k: _convert_dtype(v) for k, v in conditioning_inputs.items()}
    # Now the generative AI part:
    # k-diffusion denoising process go!
