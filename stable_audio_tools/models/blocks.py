...
class ADAASnakeBeta(nn.Module):
    ...
    def forward(self, x):
        ...
        x_shifted = torch.cat([torch.zeros_like(x[..., :1], device=x.device), x[..., :-1]], dim=-1)
        x_delta = x - x_shifted
        x_sum = x + x_shifted
        y = x_sum / 2.0 + (1.0 / (2.0 * (beta + self.no_div_by_zero))) * (1.0 - torch.cos(alpha * x_sum) * torch.sinc(alpha * x_delta / torch.pi))
        return y
