...
class MMAudioDitSingleBlock(nn.Module):
...
class MMAudioJointBlock(nn.Module):

    def __init__(self, dim: int, nhead: int, mlp_ratio: float = 4.0, pre_only: bool = False):
        super().__init__()
        self.pre_only = pre_only
        self.latent_block = MMAudioDitSingleBlock(dim,
                                             nhead,
                                             mlp_ratio,
                                             pre_only=False,
                                             kernel_size=3,
                                             padding=1)
        self.clip_block = MMAudioDitSingleBlock(dim,
                                           nhead,
                                           mlp_ratio,
                                           pre_only=pre_only,
                                           kernel_size=3,
                                           padding=1)
        self.text_block = MMAudioDitSingleBlock(dim, nhead, mlp_ratio, pre_only=pre_only, kernel_size=1)
...
class JointBlock(nn.Module):
    ...
    def __init__(self, dim: int, nhead: int, mlp_ratio: float = 4.0, pre_only: bool = False):
        ...
        self.latent_block = MMDitSingleBlock(dim,
                                             ...
                                             kernel_size=3,
                                             padding=1)
        self.low_level_block = MMDitSingleBlock(dim,
                                           nhead,
                                           mlp_ratio,
                                           pre_only=pre_only,
                                           kernel_size=3,
                                           padding=1)
        self.high_level_block = MMDitSingleBlock(dim,
                                           nhead,
                                           mlp_ratio,
                                           pre_only=pre_only,
                                           kernel_size=3,
    ...
    def forward(self, latent: torch.Tensor, low_level_cond: torch.Tensor, high_level_cond: torch.Tensor,
                time_cond: torch.Tensor, global_cond: torch.Tensor, latent_rot: torch.Tensor, low_level_rot: torch.Tensor, high_level_rot: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        # latent: BS * N1 * D
        # clip_f: BS * N2 * D
        # c: BS * (1/N) * D
        x_qkv, x_mod = self.latent_block.pre_attention(latent, global_cond, latent_rot)
        c_qkv, c_mod = self.low_level_block.pre_attention(low_level_cond, time_cond, low_level_rot)
        t_qkv, t_mod = self.high_level_block.pre_attention(high_level_cond, time_cond, high_level_rot)

        latent_len = latent.shape[1]
        low_level_len = low_level_cond.shape[1]
        high_level_len = high_level_cond.shape[1]

        joint_qkv = [torch.cat([x_qkv[i], c_qkv[i], t_qkv[i]], dim=2) for i in range(3)]

        attn_out = attention(*joint_qkv)
        x_attn_out = attn_out[:, :latent_len]
        c_attn_out = attn_out[:, latent_len:latent_len + low_level_len]
        t_attn_out = attn_out[:, latent_len + low_level_len:]

        latent = self.latent_block.post_attention(latent, x_attn_out, x_mod)
        if not self.pre_only:
            low_level_cond = self.low_level_block.post_attention(low_level_cond, c_attn_out, c_mod)
            high_level_cond = self.high_level_block.post_attention(high_level_cond, t_attn_out, t_mod)
        return latent, low_level_cond, high_level_cond
