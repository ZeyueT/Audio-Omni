...
from .blocks import SnakeBeta, ADAASnakeBeta
...
from .conv_encodec import SConv1d
...
def get_activation(activation: Literal["elu", "snake", "none", "adaa_snake"], antialias=False, channels=None) -> nn.Module:
    if activation == "elu":
        act = nn.ELU()
        ...
        act = nn.Identity()
    elif activation == "adaa_snake":
        act = ADAASnakeBeta(channels)
    else:
...
class ResidualUnit(nn.Module):
    def __init__(self, in_channels, out_channels, dilation, act_type, antialias_activation=False):
        super().__init__()
        ...
        self.layers = nn.Sequential(
            get_activation(act_type, antialias=antialias_activation, channels=out_channels),
...
class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, act_type, antialias_activation=False):
        super().__init__()

        self.layers = nn.Sequential(
            ResidualUnit(in_channels=in_channels,
                         out_channels=in_channels, dilation=1, act_type=act_type),
            ResidualUnit(in_channels=in_channels,
                         out_channels=in_channels, dilation=3, act_type=act_type),
            ResidualUnit(in_channels=in_channels,
                         out_channels=in_channels, dilation=9, act_type=act_type),
            get_activation(act_type, antialias=antialias_activation, channels=in_channels),
            WNConv1d(in_channels=in_channels, out_channels=out_channels,
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, act_type, antialias_activation=False, use_nearest_upsample=False, use_filter=False):
        super().__init__()
        ...
        self.layers = nn.Sequential(
            ...
            ResidualUnit(in_channels=out_channels, out_channels=out_channels,
                        dilation=1, act_type=act_type),
            ResidualUnit(in_channels=out_channels, out_channels=out_channels,
                        dilation=3, act_type=act_type),
            ResidualUnit(in_channels=out_channels, out_channels=out_channels,
                        dilation=9, act_type=act_type),
        )
...
class OobleckEncoder(nn.Module):
    def __init__(self, 
        ...
        for i in range(self.depth-1):
            layers += [EncoderBlock(in_channels=c_mults[i]*channels, out_channels=c_mults[i+1]*channels, stride=strides[i], act_type=act_type)]

        layers += [
            get_activation(act_type, antialias=antialias_activation, channels=c_mults[-1] * channels),
...
class OobleckDecoder(nn.Module):
    def __init__(self, 
        ...
        act_type = "elu"
        if use_snake:
            if snake_type == 'vanilla':
                act_type = "snake"
            elif snake_type == 'adaa':
                act_type = "adaa_snake"
            else:
                raise ValueError(f"Unknown snake type {snake_type}. Must be 'vanilla' or 'adaa'")
        ...
        layers += [
            get_activation(act_type, antialias=antialias_activation, channels=c_mults[0] * channels),
...
class AudioAutoencoder(nn.Module):
    ...
    def encode(self, audio, skip_bottleneck: bool = False, return_info=False, skip_pretransform=False, iterate_batch=False, **kwargs):
        ...
        # iterate_batch: 使用子批次处理，减少循环次数
        subbatch_size = max(1, audio.shape[0] // 4) if iterate_batch else audio.shape[0]

        if self.pretransform is not None and not skip_pretransform:
            if self.pretransform.enable_grad:
                if iterate_batch:
                    audios = []
                    for i in range(0, audio.shape[0], subbatch_size):
                        audios.append(self.pretransform.encode(audio[i:i+subbatch_size]))
        ...
        if self.encoder is not None:
            if iterate_batch:
                latents = []
                for i in range(0, audio.shape[0], subbatch_size):
                    latents.append(self.encoder(audio[i:i+subbatch_size]))
    ...
    def decode(self, latents, skip_bottleneck: bool = False, iterate_batch=False, **kwargs):
        ...
        # iterate_batch: 使用子批次处理，减少循环次数
        subbatch_size = max(1, latents.shape[0] // 4) if iterate_batch else latents.shape[0]

        if self.bottleneck is not None and not skip_bottleneck:
            if iterate_batch:
                decoded = []
                for i in range(0, latents.shape[0], subbatch_size):
                    decoded.append(self.bottleneck.decode(latents[i:i+subbatch_size]))
        ...
        if iterate_batch:
            decoded = []
            for i in range(0, latents.shape[0], subbatch_size):
                decoded.append(self.decoder(latents[i:i+subbatch_size]))
        ...
        if self.pretransform is not None:
            if self.pretransform.enable_grad:
                if iterate_batch:
                    decodeds = []
                    for i in range(0, decoded.shape[0], subbatch_size):
                        decodeds.append(self.pretransform.decode(decoded[i:i+subbatch_size]))
    ...
    def preprocess_audio_list_for_encoder(self, audio_list, in_sr_list):
        ...
        for i in range(batch_size):
            ...
            new_audio[i] = prepare_audio(new_audio[i], in_sr=in_sr, target_sr=self.sample_rate, target_length=padded_audio_length, 
...
def create_autoencoder_from_config(config: Dict[str, Any]):
    ...
    if ae_config.get("encoder", {}).get("type") == "dac-vae" and ae_config.get("decoder", {}).get("type") == "dac-vae":
        # DAC-VAE: encoder 和 decoder 共享同一个 VAE 实例
        ...
        print("Encoder 和 Decoder 已成功共享同一个 DAC-VAE 实例。")
    else:
