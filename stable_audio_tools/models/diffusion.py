...
from .dit import DiffusionTransformer, MMDiffusionTransformer, QwenMMDiTDiffusionTransformer, FuseMMDiffusionTransformer, MMAudioDiffusionTransformer
from .dit_latent_forcing import LatentForcingDiTWrapper
...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def __init__(
        ...
        self.gate = gate
        if gate:
            if gate_type == "sparse_gated":
                use_sparse_gating = gate_type_config.get("use_sparse_gating", True)
                use_token_gate = gate_type_config.get("use_token_gate", True)
                use_global_gate = gate_type_config.get("use_global_gate", True)
                gating_on_attention = gate_type_config.get("gating_on_attention", True)
                equal_fusion = gate_type_config.get("equal_fusion", False)
                from .DynamicSparseGatedAttn import DynamicSparseGatedXATTN_DENSE
                self.dynamic_sparse_gated_attn = DynamicSparseGatedXATTN_DENSE(d_model=768, n_heads=12, use_sparse_gating=use_sparse_gating, use_token_gate=use_token_gate, use_global_gate=use_global_gate, gating_on_attention=gating_on_attention, equal_fusion=equal_fusion)
            elif gate_type == "FuseGatedRouter":
                from .DynamicSparseGatedAttn import SplitAndFuseGatedRouter
                num_summary_queries = gate_type_config.get("num_summary_queries", 1)
                num_heads = gate_type_config.get("num_heads", 12)
                self.split_and_fuse_gated_router = SplitAndFuseGatedRouter(dim=768, num_summary_queries=num_summary_queries, num_heads=num_heads)
                
            elif gate_type == "FuseGatedRouter_wo_BypassGate":
                from .DynamicSparseGatedAttn import SplitAndFuseGatedRouter_wo_BypassGate
                num_summary_queries = gate_type_config.get("num_summary_queries", 1)
                num_heads = gate_type_config.get("num_heads", 12)
                self.split_and_fuse_gated_router_wo_BypassGate = SplitAndFuseGatedRouter_wo_BypassGate(dim=768, num_summary_queries=num_summary_queries, num_heads=num_heads)
                
            elif gate_type == "FuseGatedRouter_wo_Gate":
                from .DynamicSparseGatedAttn import SplitAndFuseGatedRouter_wo_Gate
                num_summary_queries = gate_type_config.get("num_summary_queries", 1)
                num_heads = gate_type_config.get("num_heads", 12)
                self.split_and_fuse_gated_router_wo_Gate = SplitAndFuseGatedRouter_wo_Gate(dim=768, num_summary_queries=num_summary_queries, num_heads=num_heads)
                

            elif gate_type == "FuseGatedRouter_wo_Scatter":
                from .DynamicSparseGatedAttn import SplitAndFuseGatedRouter_wo_Scatter
                num_summary_queries = gate_type_config.get("num_summary_queries", 1)
                num_heads = gate_type_config.get("num_heads", 12)
                self.split_and_fuse_gated_router_wo_Scatter = SplitAndFuseGatedRouter_wo_Scatter(dim=768, num_summary_queries=num_summary_queries, num_heads=num_heads)
                
                
            elif gate_type == "FuseGatedRouter_wo_Query":
                from .DynamicSparseGatedAttn import SplitAndFuseGatedRouter_wo_Query
                num_summary_queries = gate_type_config.get("num_summary_queries", 1)
                num_heads = gate_type_config.get("num_heads", 12)
                self.split_and_fuse_gated_router_wo_Query = SplitAndFuseGatedRouter_wo_Query(dim=768, num_summary_queries=num_summary_queries, num_heads=num_heads)
                

            elif gate_type == "MAF":
                from .DynamicSparseGatedAttn import MAF_Block
                ...
                from .DynamicSparseGatedAttn import MLF_Block
                num_experts_per_modality = gate_type_config.get("num_experts_per_modality", 64)
                num_heads = gate_type_config.get("num_heads", 12)
                num_fusion_layers = gate_type_config.get("num_fusion_layers", 4)
                self.mlf_block = MLF_Block(dim=768, num_experts_per_modality=num_experts_per_modality, num_heads=num_heads, num_fusion_layers=num_fusion_layers)


            elif gate_type == "MALF":
                self.gate_type = "MALF"
                from .DynamicSparseGatedAttn import MALF_Block
                num_experts_per_modality = gate_type_config.get("num_experts_per_modality", 64)
                num_heads = gate_type_config.get("num_heads", 12)
                num_fusion_layers = gate_type_config.get("num_fusion_layers", 4)
                self.malf_block_highlevel = MALF_Block(dim=768, num_experts_per_modality=num_experts_per_modality, num_heads=num_heads, num_fusion_layers=num_fusion_layers)
                self.malf_block_lowlevel = MALF_Block(dim=768, num_experts_per_modality=num_experts_per_modality, num_heads=num_heads, num_fusion_layers=num_fusion_layers)
                
                if gate_type_config.get("linear_global_cond", False):
                    # 这里用两个复杂一点的linear
                    import torch.nn as nn
                    linear_global_cond_dim = gate_type_config.get("linear_global_cond_dim", 550)
                    global_dim = gate_type_config.get("global_dim", 1100)
                    self.linear_global_cond = nn.Sequential(
                        nn.Linear(global_dim, 2048),
                        nn.GELU(),
                        nn.Linear(2048, linear_global_cond_dim)
                    )
    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[torch.Tensor, tp.Any], negative=False):
        ...
        if len(self.global_cond_ids) > 0:
            ...
            if hasattr(self, "gate_type") and self.gate_type == "MALF":

                m1_feature_low = global_conds[0]
                m2_feature_low = global_conds[1]
                lowlevel_refined_branches = self.malf_block_lowlevel(m1_feature_low, m2_feature_low)
                global_conds = [lowlevel_refined_branches["m1_tokens"], lowlevel_refined_branches["m2_tokens"]]


            global_cond = torch.cat(global_conds, dim=-2)
            # global_cond = torch.cat(global_conds, dim=-1)

            if hasattr(self, "linear_global_cond"):
                global_cond = self.linear_global_cond(global_cond.transpose(1, 2)).transpose(1, 2)
            ...
            if self.gate:
                ...
                elif self.gate_type == "MLF":
                    highlevel_feature = torch.cat(cross_attention_input, dim=1)
                    lowlevel_feature = global_cond
                    refined_branches = self.mlf_block(highlevel_feature, lowlevel_feature)

                    cross_attention_input = refined_branches["highlevel_tokens"]
                    global_cond = refined_branches["lowlevel_tokens"]


                    # cross_attention_input = torch.cat(list(refined_branches.values()), dim=1)
                elif self.gate_type == "MALF":
                    m1_feature_high = cross_attention_input[0]
                    m2_feature_high = cross_attention_input[1]
                    highlevel_refined_branches = self.malf_block_highlevel(m1_feature_high, m2_feature_high)
                    cross_attention_input = torch.cat([highlevel_refined_branches["m1_tokens"], highlevel_refined_branches["m2_tokens"]], dim=1)
...
class FuseMMDiTWrapper(ConditionedDiffusionModel):
    def __init__(
        self,
        *args,
        **kwargs
        ...
        self.model = FuseMMDiffusionTransformer(*args, **kwargs)
...
class MMAudioDiTWrapper(ConditionedDiffusionModel):
    def __init__(
        self,
        *args,
        **kwargs
        ...
        self.model = MMAudioDiffusionTransformer(*args, **kwargs)
...
def create_diffusion_cond_from_config(config: tp.Dict[str, tp.Any]):
    ...
    elif diffusion_model_type == 'fusemmdit':
        diffusion_model = FuseMMDiTWrapper(**diffusion_model_config)
    elif diffusion_model_type == 'mmaudiodit':
        diffusion_model = MMAudioDiTWrapper(**diffusion_model_config)
    elif diffusion_model_type == 'qwen_mmdit':
        diffusion_model = QwenMMDiTWrapper(**diffusion_model_config)
    elif diffusion_model_type == 'dit_latent_forcing':
        diffusion_model = LatentForcingDiTWrapper(**diffusion_model_config)
    ...
    elif diffusion_model_type in ("dit", "mmdit", "dit_latent_forcing"):
