...
class Attention(nn.Module):
    def __init__(
        ...
        self.use_npu_flash = True if USE_NPU_FLASH else False        
...
class ContinuousMMDiTTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        from .mm_transformer_layers import MMDitSingleBlock, JointBlock, FinalBlock
        hidden_dim = dim
        num_heads = dim_heads
        ...
        single_modal_depth = depth-fusion_depth
        ...
        self._latent_seq_len += self.global_dim
        ...
        self.layers = nn.ModuleList([
            ...
            for i in range(single_modal_depth)
        ])

        self.proj_mm_tokens = nn.Linear(768, hidden_dim) if dim != 768 else nn.Identity()
        self.proj_mm_seq_len = nn.Linear(mm_seq_len, self._latent_seq_len) if self._latent_seq_len != mm_seq_len else nn.Identity()
    ...
    def forward(
        ...
        **kwargs
        ...
        time_cond = kwargs.get("timestep_embed", None).unsqueeze(1)
        mm_tokens = context
...
class ContinuousFuseMMDiTTransformer(nn.Module):
    def __init__(
        self,
        dim,
        depth,
        ...
        **kwargs
        ...
        self.add_lowlevel_global = add_lowlevel_global
        if self.add_lowlevel_global:
            self.proj_global_cond = nn.Linear(global_dim, _latent_seq_len)
        ...
        if fusion_depth > 0:
            self.joint_blocks = nn.ModuleList([
                JointBlock(hidden_dim,
                        num_heads,
                        mlp_ratio=mlp_ratio,
                        pre_only=(i == fusion_depth - 1)) for i in range(fusion_depth)
            ])
            self.final_layer = FinalBlock(hidden_dim, _latent_seq_len)
        ...
        self.proj_high_level_tokens = nn.Linear(768, hidden_dim) if dim != 768 else nn.Identity()

        self.proj_latent_seq_len = nn.Linear(_latent_seq_len, _latent_seq_len+global_dim+mm_seq_len)
        self.proj_low_level_seq_len = nn.Linear(global_dim, _latent_seq_len+global_dim+mm_seq_len)
        self.proj_high_level_seq_len = nn.Linear(mm_seq_len, _latent_seq_len+global_dim+mm_seq_len)

        self.proj_latent_out = nn.Linear(_latent_seq_len+global_dim+mm_seq_len, _latent_seq_len)
        ...
        fusion_rot = compute_rope_rotations(self._latent_seq_len+global_dim+mm_seq_len,
                                            hidden_dim // num_heads,
                                            10000,
                                            freq_scaling=base_freq,
                                            device=self.device)
        self.register_buffer('fusion_rot', fusion_rot, persistent=False)


        low_level_rot = compute_rope_rotations(self.global_dim,
                                            hidden_dim // num_heads,
                                            10000,
                                            freq_scaling=base_freq,
                                            device=self.device)
        self.register_buffer('low_level_rot', low_level_rot, persistent=False)    

        high_level_rot = compute_rope_rotations(mm_seq_len,
                                            hidden_dim // num_heads,
                                            10000,
                                            freq_scaling=base_freq,
                                            device=self.device)
        self.register_buffer('high_level_rot', high_level_rot, persistent=False)
    ...
    def forward(
        ...
        **kwargs
        ...
        low_level_cond = prepend_embeds
        high_level_cond = context
        high_level_cond = self.proj_high_level_tokens(high_level_cond)
        time_embed = kwargs["timestep_embed"].unsqueeze(1)

        if self.add_lowlevel_global:
            global_cond = time_embed+low_level_cond
            global_cond = self.proj_global_cond(global_cond.transpose(1, 2)).transpose(1, 2)
        else:
            global_cond = time_embed

        for block in self.joint_blocks:
            x, low_level_cond, high_level_cond = block(x, low_level_cond=low_level_cond, high_level_cond=high_level_cond, time_cond=time_embed, global_cond=global_cond,
                                           latent_rot=self.latent_rot, low_level_rot=self.low_level_rot, high_level_rot=self.high_level_rot)  # (B, N, D)

        fusion = torch.cat((x, low_level_cond, high_level_cond), dim=1)
        gate_low_level = self.gated_mlp_v(fusion)
        gate_high_level = self.gated_mlp_t(fusion)

        x = self.proj_latent_seq_len(x.transpose(1, 2)).transpose(1, 2)
        low_level_cond = self.proj_low_level_seq_len(low_level_cond.transpose(1, 2)).transpose(1, 2)
        high_level_cond = self.proj_high_level_seq_len(high_level_cond.transpose(1, 2)).transpose(1, 2)
        x = x + low_level_cond * gate_low_level + high_level_cond * gate_high_level

        for block in self.layers:
            if self.cross_attend:
                x = block(x, gate_low_level, self.fusion_rot, context=high_level_cond)
            else:
                x = block(x, mm_tokens, self.fusion_rot)

        x = self.proj_latent_out(x.transpose(1, 2)).transpose(1, 2)
        x = self.project_out(x)
...
class ContinuousMMAudioDiTTransformer(nn.Module):
    def __init__(
        self,
        dim,
        depth,
        ...
        **kwargs
        ...
        from .mm_transformer_layers import MMAudioDitSingleBlock, MMAudioJointBlock, FinalBlock
        hidden_dim = dim
        num_heads = dim_heads
        mlp_ratio = kwargs.get('mlp_ratio', 4.0)
        kernel_size = kwargs.get('kernel_size', 3)
        ...
        self.layers = nn.ModuleList([
            MMAudioDitSingleBlock(hidden_dim, num_heads, mlp_ratio=mlp_ratio, kernel_size=kernel_size, padding=padding_size, cross_attend=cross_attend)
            for i in range(single_modal_depth)
        ])
        ...
        self.sync_pos_emb = nn.Parameter(torch.zeros((1, 1, 8, 768)))
    ...
    def preprocess_conditions(self, clip_f: torch.Tensor, sync_f: torch.Tensor,
        ...
        num_sync_segments = sync_f.shape[1] // 8
    ...
    def forward(
        ...
        **kwargs
        ...
        preprocessed = self.preprocess_conditions(video_f_c, text_f_c, sync_f_c)
        clip_f = preprocessed.clip_f
        text_f = preprocessed.text_f
        sync_f = preprocessed.sync_f
        clip_f_c = preprocessed.clip_f_c
        text_f_c = preprocessed.text_f_c

        global_c=self.global_cond_mlp(clip_f_c + text_f_c)
        global_c = time_embed+global_c.unsqueeze(1)
        extended_c = global_c + sync_f

        for block in self.joint_blocks:
            x, clip_f, text_f = block(x, clip_f, text_f, global_c, extended_c,
                                           self.latent_rot, self.clip_rot)  # (B, N, D)

        for block in self.layers:
            if self.cross_attend:
                x = block(x, extended_c, self.latent_rot, context=text_f)
            else:
                x = block(x, extended_c, self.latent_rot)
